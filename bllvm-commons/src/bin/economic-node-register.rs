...
fn register_node(
    ...
    match node_type {
        ...
        "exchange" | "custodian" => {
            if economic_activity_percent.is_none() {
                return Err(
                    "Exchanges and custodians must specify economic_activity_percent".into(),
                );
            }
    ...
    println!(
        "  scp {} governance-server:/path/to/economic-registrations/",
        registration_file
    );
...
fn generate_keypair(name: &str, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn submit_proof(
    ...
    fs::write(
        &proof_file,
        serde_json::to_string_pretty(&proof_submission)?,
    )?;
...
fn check_status(name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if fs::metadata(&registration_file).is_ok() {
        let registration: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&registration_file)?)?;
        ...
        if registration["economic_activity_percent"].is_number() {
            println!(
                "  Economic Activity: {}%",
                registration["economic_activity_percent"]
            );
        }
    } else {
