...
async fn main() -> Result<()> {
    ...
    let nsec = env::var(&nsec_var)
        .map_err(|_| anyhow::anyhow!("Environment variable {} not set. Make sure the GitHub secret is configured.", nsec_var))?;

    // Parse relays
    let relays: Vec<String> = args.relays
        .unwrap_or_else(|| env::var("NOSTR_RELAYS").unwrap_or_else(|_| "wss://relay.damus.io,wss://nos.lol".to_string()))
        .split(',')
    ...
    let mut tags = vec![
        Tag::Generic(
            ...
            vec![args.event_type.clone()],
        ),
        Tag::Generic(
            TagKind::Custom("bot".into()),
            vec![args.bot.clone()],
        ),
    ];
    ...
    tracing::info!("✅ Successfully published to {}/{} relays", published, relays_list.len());
