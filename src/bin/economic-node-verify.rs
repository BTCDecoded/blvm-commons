...
fn verify_registration(name: &str, file: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
...
fn verify_veto(file: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn verify_all_registrations(dir: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn verify_all_vetoes(dir: &str) -> Result<(), Box<dyn std::error::Error>> {
                ...
                if let Ok(veto) = serde_json::from_str::<serde_json::Value>(&content) {
                        ...
                        && veto["pr_number"].is_number()
                    {
                        ...
                        if veto["active"] == true {
                            ...
                            println!(
                                "✅ {} (active)",
                                veto["node_name"].as_str().unwrap_or("unknown")
                            );
                        } else {
                            println!(
                                "✅ {} (withdrawn)",
...
fn check_governance_status(repo: &str, pr: u64) -> Result<(), Box<dyn std::error::Error>> {
