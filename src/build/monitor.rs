...
impl BuildMonitor {
    ...
    pub async fn monitor_build(
        ...
        info!(
            "Monitoring build for {}/{} (run ID: {})",
            self.organization, repo, workflow_run_id
        );
        ...
        loop {
            ...
            match self.get_workflow_run_status(repo, workflow_run_id).await {
                ...
                Err(e) => {
                    error!(
                        "Error checking build status for {}/{}: {}",
                        self.organization, repo, e
                    );
                    sleep(self.poll_interval).await;
                }
            }
    ...
    async fn get_workflow_run_status(
        ...
        self.get_workflow_run_status_by_id(repo, workflow_run_id)
            .await
    }
    ...
    async fn get_workflow_run_status_by_id(
        ...
        let conclusion = run_status.get("conclusion").and_then(|v| v.as_str());
