...
impl ArtifactCollector {
    ...
    pub async fn download_artifacts(
        ...
        for artifact in artifacts.iter_mut() {
            ...
            if let Some(expires_at) = artifact.expires_at {
                ...
                if now > expires_at {
                    warn!(
                        "Skipping expired artifact '{}' (expired at {})",
                        artifact.name, expires_at
                    );
                    continue;
                }
                let time_until_expiry = expires_at - now;
                info!(
                    "Artifact '{}' expires in {} hours",
                    artifact.name,
    ...
    pub async fn download_all_artifacts(
        ...
        info!(
            "Downloading artifacts from {} repositories",
            all_artifacts.len()
        );
