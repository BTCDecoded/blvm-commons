...
impl VoteAggregator {
    ...
    pub async fn aggregate_proposal_votes(
        ...
        let participation_votes = self.get_participation_votes(pr_id, tier as u32).await?;
    ...
    pub async fn check_economic_veto_blocking(&self, pr_id: i32, tier: u8) -> Result<bool> {
        ...
        let veto_threshold = self
            .veto_manager
            .check_veto_threshold(pr_id, tier as u32)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to check veto threshold: {}", e))?;

        // Veto blocks if: threshold met (AND logic: both mining and economic thresholds required)
        // Veto is active if threshold met and not overridden
        Ok(veto_threshold.veto_active)
    }
