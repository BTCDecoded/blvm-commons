...
impl GovernancePhaseCalculator {
    ...
    /// Create with ConfigReader for governance-controlled phase boundaries
    ...
    pub async fn get_current_phase(&self) -> Result<GovernancePhase, GovernanceError> {
        ...
        // Calculate phase for each metric (using config if available)
        let height_phase = self.determine_phase_by_height(block_height).await?;
        let node_phase = self.determine_phase_by_nodes(economic_nodes).await?;
        let contributor_phase = self.determine_phase_by_contributors(contributors).await?;
    ...
    /// Determine phase by block height (with config support)
    async fn determine_phase_by_height(&self, height: u64) -> Result<GovernancePhase, GovernanceError> {
        if let Some(ref config) = self.config {
            let early_max = config.get_phase_boundaries("early", "max_blocks").await?;
            let growth_max = config.get_phase_boundaries("growth", "max_blocks").await?;
            
            if height < early_max {
                Ok(GovernancePhase::Early)
            } else if height < growth_max {
                Ok(GovernancePhase::Growth)
            } else {
                Ok(GovernancePhase::Mature)
            }
        } else {
    ...
    /// Determine phase by economic node count (with config support)
    async fn determine_phase_by_nodes(&self, count: u32) -> Result<GovernancePhase, GovernanceError> {
        if let Some(ref config) = self.config {
            let early_max = config.get_phase_boundaries("early", "max_nodes").await?;
            let growth_max = config.get_phase_boundaries("growth", "max_nodes").await?;
            
            if count < early_max as u32 {
                Ok(GovernancePhase::Early)
            } else if count < growth_max as u32 {
                Ok(GovernancePhase::Growth)
            } else {
                Ok(GovernancePhase::Mature)
            }
    ...
    /// Determine phase by contributor count (with config support)
    async fn determine_phase_by_contributors(&self, count: u32) -> Result<GovernancePhase, GovernanceError> {
        if let Some(ref config) = self.config {
            let early_max = config.get_phase_boundaries("early", "max_contributors").await?;
            let growth_max = config.get_phase_boundaries("growth", "max_contributors").await?;
