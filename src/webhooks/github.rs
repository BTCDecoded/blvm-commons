...
use crate::webhooks::{comment, pull_request, review, release};
...
pub async fn handle_webhook(
    ...
    info!("Received webhook: event_type={}, action={}", event_type, action);
        ...
        "release" => {
            ...
            let github_client = match GitHubClient::new(
                ...
                Err(e) => {
                    ...
                    return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "failed to initialize"})));
                }
            };
            
            // Extract organization from governance_repo (format: "org/repo")
            let organization = config.governance_repo
                .split('/')
                .next()
            ...
            match release::handle_release_event(&payload, &orchestrator).await {
                ...
                Err(e) => {
                    ...
                    (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "failed"})))
                }
            }
        }
        "repository_dispatch" => {
