...
pub async fn handle_webhook(
    State((config, database, config_registry)): State<(
        ...
        "pull_request" => {
            match action {
                ...
                "closed" => {
                    ...
                    if merged {
                        // PR was merged - publish to Nostr and activate config changes if Tier 5
                        if let Err(e) = pull_request::handle_pr_merged(
                            &config,
                            &database,
                            &payload,
                            Some(config_registry.clone()),
                        )
                        .await
                        {
                            warn!("Failed to process PR merge: {}", e);
                        }
                    }
