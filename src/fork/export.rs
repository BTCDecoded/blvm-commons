...
impl GovernanceExporter {
    ...
    /// Version 2.0: Includes all material governance dimensions
    pub async fn export_governance_config(
        ...
        // Load all governance configuration files (core dimensions)
        ...
        // Load expanded dimensions (Version 2.0)
        let nested_multisig_teams = self.load_config_file_optional("maintainers/teams.yml").await?;
        let governance_review_policy = self.load_config_file_optional("governance-review-policy.yml").await?;
        let repository_layers = self.load_config_file_optional("repository-layers.yml").await?;
        let tier_classification = self.load_config_file_optional("tier-classification-rules.yml").await?;
        let emergency_procedures = self.load_config_file_optional("emergency-tiers.yml").await?;
        let cross_layer_rules = self.load_config_file_optional("cross-layer-rules.yml").await?;
        let commons_contributor_thresholds = self.load_config_file_optional("commons-contributor-thresholds.yml").await?;
        let validation_requirements = self.load_config_file_optional("validation-requirements.yml").await?;

        // Create configuration hash (include all dimensions)
        let config_data = serde_json::json!({
            ...
            "nested_multisig_teams": nested_multisig_teams,
            "governance_review_policy": governance_review_policy,
            "repository_layers": repository_layers,
            "tier_classification": tier_classification,
            "emergency_procedures": emergency_procedures,
            "cross_layer_rules": cross_layer_rules,
            "commons_contributor_thresholds": commons_contributor_thresholds,
    ...
    /// Load a single configuration file optionally (returns None if file doesn't exist)
    async fn load_config_file_optional(&self, filename: &str) -> Result<Option<Value>, GovernanceError> {
        let file_path = Path::new(&self.config_path).join(filename);
