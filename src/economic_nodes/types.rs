    ...
    pub fn qualification_thresholds(&self) -> QualificationThresholds {
        match self {
            ...
            NodeType::Exchange => QualificationThresholds {
                ...
                minimum_volume_btc: Some(100.0), // 100 BTC daily volume (on-chain verifiable)
                minimum_transactions_monthly: None,
                minimum_merge_mining_btc: None,
                minimum_fee_forwarding_btc: None,
                minimum_zap_quantity_btc: None,
...
pub struct QualificationThresholds {
    ...
    pub minimum_volume_btc: Option<f64>, // BTC volume (on-chain verifiable)
...
pub struct VetoSignal {
    pub id: Option<i32>,
    pub pr_id: i32,
    pub node_id: i32, // Internal tracking only (not exposed in Merkle proof)
    ...
    pub verified: bool,
    /// Voting key public key (derived from registration key via BIP32)
    /// Used for privacy-preserving veto signals
    #[serde(default)]
    pub voting_public_key: Option<String>,
    /// Voting key derivation path (e.g., "m/0'/123'/0'")
    /// Used to verify voting key is derived from registration key
    #[serde(default)]
    pub voting_key_path: Option<String>,
    /// Signal index (for key derivation, starts at 0)
    #[serde(default)]
    pub signal_index: Option<u32>,
}
...
pub struct VolumeProof {
    pub daily_volume_btc: f64, // Daily BTC volume (on-chain verifiable)
    pub monthly_volume_btc: f64, // Monthly BTC volume (on-chain verifiable)
    pub transaction_hashes: Vec<String>, // On-chain transaction hashes for verification
}
