...
impl VetoManager {
    ...
    pub fn with_config(pool: SqlitePool, config: Arc<ConfigReader>) -> Self {
        Self {
            pool: pool.clone(),
            signature_manager: SignatureManager::new(),
            phase_calculator: Some(GovernancePhaseCalculator::new(pool.clone())),
    ...
    /// Collect a veto signal from an economic node (legacy method, uses registration key)
    pub async fn collect_veto_signal(
        ...
        self.collect_veto_signal_with_voting_key(
            pr_id,
            node_id,
            signal_type,
            signature,
            rationale,
            None, // No voting key (use registration key)
            None, // No voting key path
            None, // No signal index
        )
        .await
    }

    /// Collect a veto signal from an economic node with privacy-preserving voting key
    ///
    /// If voting_public_key is provided, the signature must be signed with the voting private key.
    /// The voting key is derived from the registration key via BIP32 (client-side).
    /// This allows nodes to prove their signal was counted without revealing their identity.
    pub async fn collect_veto_signal_with_voting_key(
        &self,
        pr_id: i32,
        node_id: i32,
        signal_type: SignalType,
        signature: &str,
        rationale: &str,
        voting_public_key: Option<&str>,
        voting_key_path: Option<&str>,
        signal_index: Option<u32>,
    ) -> Result<i32, GovernanceError> {
        // Get node information
        let registry = if let Some(ref phase_calc) = self.phase_calculator {
            EconomicNodeRegistry::with_phase_calculator(self.pool.clone(), Some(phase_calc.clone()))
        } else {
            EconomicNodeRegistry::new(self.pool.clone())
        };
        ...
        // Verify signature
        // If voting key is provided, verify signature with voting key
        // Otherwise, verify with registration key (legacy)
        let verified = if let Some(voting_key) = voting_public_key {
            // Verify signature with voting key
            let message = format!("PR #{} veto signal", pr_id);
            ...
            // Legacy: verify with registration key
        ...
        // If using voting key, check by voting key; otherwise check by node_id
        let existing = if let Some(voting_key) = voting_public_key {
            sqlx::query("SELECT id FROM veto_signals WHERE pr_id = ? AND voting_public_key = ?")
                .bind(pr_id)
                .bind(voting_key)
            ...
            sqlx::query("SELECT id FROM veto_signals WHERE pr_id = ? AND node_id = ?")
                .bind(pr_id)
                .bind(node_id)
                .fetch_optional(&self.pool)
                .await
        ...
        // Get signal index (for key derivation)
        let signal_index = signal_index.unwrap_or(0);

        // Insert veto signal with voting key fields
        let result = sqlx::query(
            r#"
            INSERT INTO veto_signals 
            (pr_id, node_id, signal_type, weight, signature, rationale, timestamp, verified,
             voting_public_key, voting_key_path, signal_index)
            VALUES (?, ?, ?, ?, ?, ?, ?, TRUE, ?, ?, ?)
            "#,
        )
        .bind(pr_id)
        ...
        .bind(voting_public_key)
        .bind(voting_key_path)
        .bind(signal_index as i32)
        .execute(&self.pool)
        .await
        .map_err(|e| {
        ...
        info!(
            "Collected {} signal from node {} for PR {} (voting_key: {})",
            signal_type.as_str(),
            node.entity_name,
            pr_id,
            voting_public_key.unwrap_or("none")
        );
    ...
    pub async fn check_veto_threshold(&self, pr_id: i32, tier: u32) -> Result<VetoThreshold, GovernanceError> {
        ...
        // Get base tier-specific thresholds from config registry (with fallback)
        let (base_mining_threshold, base_economic_threshold, review_days_default) = if let Some(ref config) = self.config {
            // Try to get from config registry
            match config.get_veto_thresholds(tier).await {
                Ok((mining, economic)) => {
                    // Get review period
                    let review_days = if tier == 4 {
                        // Tier 4 uses hours, not days
                        config.get_u32("veto_tier_4_window_hours", 24).await.unwrap_or(24) as u32
                    } else {
                        config.get_tier_review_period(tier).await.unwrap_or_else(|_| {
                            match tier {
                                3 => 90,
                                5 => 180,
                                _ => 90,
            ...
            // No config, use hardcoded defaults
            match tier {
    ...
    /// Generate Merkle proof for a specific voting key's signal on a PR
    ///
    /// Returns the Merkle proof that proves the signal was counted in the Merkle tree.
    /// This allows nodes to prove their signal was counted without revealing their identity.
    pub async fn get_merkle_proof_for_voting_key(
        &self,
        pr_id: i32,
        voting_public_key: &str,
    ) -> Result<Option<crate::governance::merkle_veto::MerkleProof>, GovernanceError> {
        use crate::governance::merkle_veto;

        // Get all verified signals for this PR
        let signals = self.get_pr_veto_signals(pr_id).await?;

        // Filter to only signals with voting keys
        let signals_with_keys: Vec<_> = signals
            .into_iter()
            .filter(|s| s.voting_public_key.is_some())
            .collect();

        if signals_with_keys.is_empty() {
            return Ok(None);
        }

        // Build Merkle tree and get proofs
        let (_root_hash, proofs) = merkle_veto::build_veto_merkle_tree(&signals_with_keys)?;

        // Return proof for this voting key
        Ok(proofs.get(voting_public_key).cloned())
    }

    /// Get Merkle root hash for a PR's veto signals
    ///
    /// Returns the root hash of the Merkle tree containing all counted veto signals for this PR.
    pub async fn get_merkle_root_hash(
        &self,
        pr_id: i32,
        ...
        let (root_hash, _proofs) = merkle_veto::build_veto_merkle_tree(&signals_with_keys)?;

        Ok(Some(hex::encode(root_hash)))
    }
