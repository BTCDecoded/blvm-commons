...
impl EmergencyTier {
    ...
    /// Get review period in days for this tier (hardcoded default)
    ...
    /// Get review period in days (with config support)
    pub async fn review_period_days_with_config(
        &self,
        config: Option<&ConfigReader>,
    ) -> Result<u32, GovernanceAppError> {
        if let Some(config) = config {
            let tier_num = self.to_i32() as u32;
            config
                .get_emergency_tier_config(tier_num, "review_period_days")
                .await
                .or_else(|_| Ok(self.review_period_days()))
        } else {
            Ok(self.review_period_days())
        }
    }

    /// Get signature threshold (N-of-M) for this tier (hardcoded default)
    ...
    /// Get signature threshold (with config support)
    pub async fn signature_threshold_with_config(
        &self,
        config: Option<&ConfigReader>,
    ) -> Result<(u32, u32), GovernanceAppError> {
        if let Some(config) = config {
            let tier_num = self.to_i32() as u32;
            let sig_threshold = config
                .get_emergency_tier_config(tier_num, "signature_threshold")
                .await
                .unwrap_or_else(|_| {
                    match self {
                        EmergencyTier::Critical => 4,
                        EmergencyTier::Urgent => 5,
                        EmergencyTier::Elevated => 6,
                    }
                });
            Ok((sig_threshold, 7)) // Total is always 7
        } else {
            Ok(self.signature_threshold())
        }
    }
    ...
    /// Get activation threshold (with config support)
    pub async fn activation_threshold_with_config(
        &self,
        config: Option<&ConfigReader>,
    ) -> Result<(u32, u32), GovernanceAppError> {
        if let Some(config) = config {
            let tier_num = self.to_i32() as u32;
            let activation_threshold = config
                .get_threshold_pair(
                    &format!("emergency_tier_{}_activation_threshold", tier_num),
                    (5, 7),
                )
                .await?;
            Ok(activation_threshold)
        } else {
            Ok(self.activation_threshold())
        }
    }

    /// Get maximum duration in days (hardcoded default)
    ...
    /// Get maximum duration in days (with config support)
    pub async fn max_duration_days_with_config(
        &self,
        config: Option<&ConfigReader>,
    ) -> Result<u32, GovernanceAppError> {
        if let Some(config) = config {
            let tier_num = self.to_i32() as u32;
            config
                .get_emergency_tier_config(tier_num, "max_duration_days")
                .await
                .or_else(|_| Ok(self.max_duration_days()))
        } else {
            Ok(self.max_duration_days())
        }
    }
    ...
    /// Get maximum number of extensions allowed (hardcoded default)
    ...
    /// Get maximum number of extensions allowed (with config support)
    pub async fn max_extensions_with_config(
        &self,
        config: Option<&ConfigReader>,
    ) -> Result<u32, GovernanceAppError> {
        if let Some(config) = config {
            let tier_num = self.to_i32() as u32;
            config
                .get_emergency_tier_config(tier_num, "max_extensions")
                .await
                .or_else(|_| Ok(self.max_extensions()))
        } else {
            Ok(self.max_extensions())
        }
    }

    /// Get extension duration in days (hardcoded default)
    ...
    /// Get extension threshold (N-of-M) (hardcoded default)
